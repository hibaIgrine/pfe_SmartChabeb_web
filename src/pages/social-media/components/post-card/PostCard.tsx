...
export function PostCard({
  ...
  const [viewingOriginalPostId, setViewingOriginalPostId] = useState<string | null>(null);
  ...
  const openOriginalSharedPost = async () => {
    ...
    try {
      ...
      const matched = candidates.find((candidate) => {
        const candidateAuthor = `${candidate.user?.nom ?? ""} ${candidate.user?.prenom ?? ""}`
          .trim()
          .toLowerCase();
  ...
  return (
    <>
      ...
      {sharedOpenError ? (
        <p className="mt-2 text-xs font-semibold text-[#a4552a]">{sharedOpenError}</p>
      ) : null}
          ...
          <div className="w-full max-w-lg rounded-2xl border border-[#e7dfcf] bg-white shadow-2xl">
            ...
            <div className="space-y-3 p-4">
              <p className="text-xs text-gray-500">
                Ajoutez un message personnel au-dessus de la publication partagée.
              </p>
              <textarea
                value={shareMessage}
