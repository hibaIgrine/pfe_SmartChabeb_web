...
export const EditClubModal = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (formData.categorie && !categories.some((c: any) => c.id === formData.categorie)) {
  ...
  }, [isOpen, formData.id_salle, formData.logo_url, formData.categorie, salles]);
    ...
    <div className="fixed inset-0 z-[600] flex items-center justify-center bg-[#1A1C1E]/65 backdrop-blur-md p-4 overflow-y-auto">
      <div className="bg-[#F7F3E9] rounded-[40px] p-8 w-full max-w-3xl shadow-2xl relative border-4 border-white animate-in zoom-in my-8 max-h-[92vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-3 mb-8 pb-5 border-b-2 border-white/60">
          <div className="w-10 h-10 bg-smart-teal/10 rounded-xl flex items-center justify-center text-smart-teal text-lg font-black">✏️</div>
          <div>
            <h2 className="text-3xl font-black text-smart-teal tracking-tight">Modifier le Club</h2>
            <p className="text-gray-400 text-xs font-bold mt-0.5">Mettez à jour les informations du club</p>
          </div>
        </div>
